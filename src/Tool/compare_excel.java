...
/**
 * 【输入】(1) pmd-1.csv (2) pmd-2.csv 【输出】 pmd-final.txt(未冗余数据清理) 【时间】2016.7.10
 * ,2016-9-5 (修改成批量处理分析),2016-10-7(前后添加“”,为清理做铺垫) 【编程思路】
 * (1).读file1的{File}列装进set中 (2).读file2的{File}列装进set2中 (3).求非交集
 * (4).排查分为两个部分，【1】非交集部分【2】交集部分少或多的部分 (5).两个文件重新一点点载入 【1】
 * 判断哪里的非交集，文字标红【在file3中写出来】 【2】 file2 的 {Line+Description}与file1 的对应部分相对比
 * 不一样的在file2中标出 set中剩下的在file1中标出 【在另一个文件file3中写出来】
 */
				...
				if (isInSet(s[2], 1)) {
					...
					if (str2 != null) {
						...
						if (s2[2].equals(s[2])) {
							...
							// 【2.1.2】不一样
							// 检测 reader_file2中的File下的String是否在set2中 记录【增加的code
							// semll】+行信息
