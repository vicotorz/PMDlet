...
public class count_code_smell_type {
	...
	public final String outpath = "D:\\junit4_total_code_smell_type.txt";
	// 检测到的放入到集合中
	Set<String> s;
	...
	public void put_into_set(String path) {
		...
		try {
			...
			CsvReader rd = new CsvReader(path, ',', Charset.forName("GBK"));
				...
				while (index < s.size()) {
					...
					if (str_num[index][0].equals(rd.getValues()[4].toString())) {
						if (!rd.getValues()[5].equals("")) {
							str_num[index][1] = String.valueOf(
									Integer.valueOf(rd.getValues()[5].toString()) + Integer.valueOf(str_num[index][1]));
							str_num[index][2] = String.valueOf(
									Integer.valueOf(rd.getValues()[9].toString()) + Integer.valueOf(str_num[index][2]));
	...
	public void sortArray() {
		...
		for (int i = 0; i < s.size(); i++) {
			System.out.print(str_num[i][0] + "   " + str_num[i][1] + "   " + str_num[i][2] + "   " + str_num[i][3]
					+ "   " + str_num[i][4]);
			System.out.println();
		}
	}
	...
	public static void main(String[] args) {
		...
		ccst.put_into_set(ccst.intopath);
		ccst.sortArray();
		ccst.WriteIntoExcel();
		ccst.output_into_file(ccst.outpath);
