...
/**
 * 2016-9-8 计算两个字符串匹配次数 【】【】,“Package” ,”File”,”[Priority] ” ,
 ...
 * 同时计算差值
 * 
 * 注意：！！！里面有，干扰 缺少明显delete的删除 缺少没有file1，或file2对应的情况
 * */
public class match_String {
	...
	public int compare_String(String str1, String str2) {
		...
		if (s1[5].length() == s2[5].length()
				|| Math.abs(s1[5].length() - s2[5].length()) <= 5) {
			// 将数字替换成空格
	...
	public void do_List() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ((loc == 0)/*没有file2,全是file1*/
					|| (loc == list.get(i).size()-1 && list.get(i).get(loc)
							.contains("【file2】【增加的code smell】"))/*没有file1,全是file2*/) {
				// 没有file1 说明全是增加的
				// 没有file2 说明全是减少的
				...
				for (int j = 0; j < loc; j++) {
					...
					for (int k = loc; k < list.get(i).size(); k++) {
						//System.out.println(compare_String(list.get(i).get(j),
							//	list.get(i).get(k)));
						if (compare_String(list.get(i).get(j),
								list.get(i).get(k)) == 6) {
							// 如果相同被标记
							list.get(i).set(j,
									list.get(i).get(j).concat("  【delete】"));
							list.get(i).set(k,
									list.get(i).get(k).concat("  【delete】"));
						}
					}
					//System.out.println();
...
//		mt.doFile("E://fastjson新工程（修改）/" + 23 + "/pmd-final.txt");
//		mt.do_List();
//		mt.Rewrite("E://fastjson新工程（修改）/" + 23 + "/pmd-final-new-new.txt");
		for (int fn = 1; fn <=28; fn++) {
			System.out.println("处理文件"+fn+"中...");
			match_String mt = new match_String();
			//C://Users/dell/Desktop/fastjsonnot/
			//E://junit4/
			mt.doFile("E://junit4not/" + fn + "/pmd-final.txt");
			...
			// }
			//
			mt.Rewrite("E://junit4not/" + fn + "/pmd-final-new.txt");
